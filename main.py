...
def create_model(model, output_head, args):
    layers_per_partition = 3
    print(layers_per_partition)

    # generate the NN for partition 0
    partition_index = 0
    device_0 = torch.device("cuda:" + str(args.local_rank))
    model_part0 = nn.Sequential(
        model.transformer.embeddings
    )
    # 0, 1, 2
    layer_start = partition_index * layers_per_partition
    layer_end = (partition_index + 1) * layers_per_partition
    for layer_index in range(layer_start, layer_end):
        print("layer_index = %d" % layer_index)
        layer_block = model.transformer.encoder.layer[layer_index]
        model_part0.add_module("transformer_block" + str(layer_index), layer_block)
    model_part0.to(device_0)

    # generate the NN for partition 1
    partition_index = 1
    device_1 = torch.device("cuda:" + str(args.local_rank + 1))
    ...
    for layer_index in range(layer_start, layer_end):
        layer_block = model.transformer.encoder.layer[layer_index]
        model_part1.add_module("transformer_block" + str(layer_index), layer_block)
    model_part1.to(device_1)

    # generate the NN for partition 1
    partition_index = 2
    device_2 = torch.device("cuda:" + str(args.local_rank + 2))
    ...
    for layer_index in range(layer_start, layer_end):
        layer_block = model.transformer.encoder.layer[layer_index]
        model_part2.add_module("transformer_block" + str(layer_index), layer_block)
    model_part2.to(device_2)

    # generate the NN for partition 2
    partition_index = 3
    device_3 = torch.device("cuda:" + str(args.local_rank + 3))
    ...
    for layer_index in range(layer_start, layer_end):
        layer_block = model.transformer.encoder.layer[layer_index]
        model_part3.add_module("transformer_block" + str(layer_index), layer_block)
    model_part3.add_module("encoder_norm", model.transformer.encoder.encoder_norm)
    model_part3.add_module("head", output_head)
    model_part3.to(device_3)
    ...
    print(pipe_model)
    # model_part0 output: x, weights
    # model_part1 input: x
    return pipe_model, device_0, device_3
...
def init_rpc(args, rank, world_size):
    # RPC address
    if args.is_infiniband:
        MASTER_ADDR = "192.168.11.1"
    else:
        MASTER_ADDR = "192.168.1.1"
    os.environ.update({"MASTER_ADDR": MASTER_ADDR})
    os.environ.update({"MASTER_PORT": "10638"})

    rpc_backend_options = TensorPipeRpcBackendOptions()
    rpc_backend_options.init_method = 'tcp://' + MASTER_ADDR + ':10639'
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="PyTorch Pipe plus DDP")
    ...
    parser.add_argument('--batch_size', type=int, default=480, metavar='N',
    ...
    # RPC
    init_rpc(args, args.local_rank, 1)
    ...
    output_head = OutputHead(output_dim)
    # split the model into a pipe sequential
    model, device_first, device_last = create_model(model, output_head, args)

    # create pipe
    checkpoint = "never"
    model = Pipe(model, chunks=8, checkpoint=checkpoint)
    print(model)
    ...
    # start training
    train_and_eval(model, train_dl, test_dl, args, device_first, device_last)
