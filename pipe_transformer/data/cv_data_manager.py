...
class CVDatasetManager(BaseDataManager):
    ...
    def get_data(self, args, dataset, node_num=0, nproc_per_node=0, node_rank=-1):
        ...
        if dataset == "cifar10":
            train_dataset, test_dataset, output_dim = self.load_cifar_centralized_training_for_vit(args, node_num,
                                                                                                   nproc_per_node,
                                                                                                   node_rank)
            ...
            train_dataset, test_dataset, output_dim = self.load_imagenet_centralized_training_for_vit(args, node_num,
                                                                                                      nproc_per_node,
                                                                                                      node_rank)
        else:
...
def test_single_worker():
    ...
    for epoch in range(data_manager.num_train_epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            logging.info(
                "---------------------------batch_idx = %d, sample_uid_list = %s" % (batch_idx, str(sample_uid_list)))
            sample_uid_list = sample_uid_list.cpu().detach().numpy()
