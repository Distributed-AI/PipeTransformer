...
class AutoCache:
    def __init__(self, auto_dp, auto_pipe, hidden_feature_size):
        ...
        self.cache_manager_train = OneLevelCache()
        self.cache_manager_test = OneLevelCache()
    ...
    def reset(self, num_frozen_layers, batch_num_train, batch_num_test):
        ...
        self.cache_manager_train.reset_status(False, self.batch_num_train,
                                              self.hidden_feature_size, self.auto_dp.get_active_world_size())
        self.cache_manager_test.reset_status(False, self.batch_num_test,
                                             self.hidden_feature_size, self.auto_dp.get_active_world_size())
        ...
        if self.is_enable:
            if frozen_model is not None:
                ...
                with torch.no_grad():
                    ...
                    hidden_feature = self.cache_manager_train.get_hidden_feature(epoch, batch_idx, x, frozen_model).to(
                        device_idx_start)
    ...
    def infer_test(self, frozen_model, pipe_model, x, batch_idx, epoch):
        if self.is_enable:
            if frozen_model is not None:
                with torch.no_grad():
                    device_idx_start = self.auto_dp.get_local_rank() * self.auto_pipe.get_pipe_len()
                    hidden_feature = self.cache_manager_test.get_hidden_feature(epoch, batch_idx, x, frozen_model).to(
                        device_idx_start)
                log_probs = pipe_model(hidden_feature)
            else:
                log_probs = pipe_model(x)
